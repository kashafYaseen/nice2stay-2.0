<template>
  <div>
    <div class="modal-datepicker-trigger">
      <button class="btn btn-outline-primary w-100" id="modal-datepicker-trigger">{{ formatDates(check_in, check_out) }}</button>
      <airbnb-style-datepicker
        :trigger-element-id="'modal-datepicker-trigger'"
        :date-one="check_in"
        :date-two="check_out"
    ...
    methods: {
      formatDates(dateOne, dateTwo) {
        ...
        if (!dateTwo && !dateOne){
          ...
          $('#modal-datepicker-trigger').addClass('btn-outline-primary');
          $('#modal-datepicker-trigger').removeClass('btn-primary');
        }

        if (dateOne) {
          formattedDates =  `${format(dateOne, this.dateFormat)} - ${this.check_out_title}`
          $('#modal-datepicker-trigger').removeClass('btn-outline-primary');
          $('#modal-datepicker-trigger').addClass('btn-primary');
        }
