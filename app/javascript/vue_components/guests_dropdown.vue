<template>
  <div
    class="dropdown guests-dropdown vue-guests-dropdown"
    :id="'vue-' + this.dropdownId"
    ref="vuedropdwon"
  >
    <button
    ...
    <div
      :class="inline ? 'position-relative' : 'dropdown-menu'"
      class="w-100"
    ...
    >
      <div :class="inline ? '' : 'dropdown-item'" class="mt-3 mb-3">
        <div class="row">
          ...
          <div :class="inline ? 'vnis mx-auto' : 'vnis'">
            <button
              :class="
                inline ? 'vnis__button col-6 bg-white' : 'vnis__button col-6'
              "
              type="button"
              @click="updateAdults('decrement')"
            >
              -
            </button>
            <input
              :class="inline ? 'vnis__input bg-secondary-dark' : 'vnis__input'"
              type="text"
              v-model="totalAdults"
            />
            <button
      ...
      <div :class="inline ? '' : 'dropdown-item'" class="mt-3 mb-3">
        <div class="row">
          <label class="col-6 text-lg pt-2"
            >{{ childrenTitle() }}<br /><span
              class="text-xxs text-capitalize"
              >{{ childrenAges() }}</span
            ></label
          >
          <div :class="inline ? 'vnis mx-auto' : 'vnis'">
            <button
              ...
              "
              type="button"
              @click="updateChildren('decrement')"
            >
              -
            </button>
            <input
      ...
      >
        <div class="row">
          ...
          <number-input-spinner
            ...
            @input="handleInfants"
            :inputClass="
              inline ? 'vnis__input bg-secondary-dark' : 'vnis__input'
            "
      ...
      <div
        :class="inline ? '' : 'dropdown-item'"
        class="mt-3 mb-3"
        v-show="this.showApply"
      >
        <input
          type="button"
          name="done"
          class="btn btn-sm btn-secondary float-right mr-0"
          value="Done"
...
export default {
  ...
  data() {
    return {
      ...
      maxCalculatedChildren:
        this.maxAdults - this.totalAdults + this.maxChildren,
    };
  },
  mounted() {
  ...
  methods: {
    updateAdults(operation = "increment") {
      if (operation == "increment") {
        if (this.totalAdults >= 0 && this.totalAdults < this.maxAdults)
          this.totalAdults += 1;
      } else {
        if (this.totalAdults > 0 && this.totalAdults <= this.maxAdults)
          this.totalAdults -= 1;
    ...
    updateChildren(operation = "increment") {
      if (operation == "increment") {
        if (
          ...
          this.totalChildren < this.maxChildren + this.maxAdults
        )
    ...
    guests() {
      ...
      if (this.totalInfants) {
        guestsTitle += `${
          this.totalChildren || this.totalAdults ? ", " : " "
        } ${this.totalInfants} ${this.infantsTitle()}`;
      }
    ...
    handleButtonClick() {
      if (!this.inline) $(`#vue-${this.dropdownId}`).dropdown("toggle");
