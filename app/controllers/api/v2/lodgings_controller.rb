class Api::V2::LodgingsController < Api::V2::ApiController
  ...
  def cumulative_price
    lodgings = Lodging.where(id: ids).includes(room_rates: %i[parent_lodging rate_plan child_lodging])
    ...
    render json: Api::V2::LodgingCumulativePriceSerializer.new(lodgings, { params: { adults: params[:adults], children: params[:children], nights: (params[:check_out].to_date - params[:check_in].to_date).to_f, check_in: params[:check_in], check_out: params[:check_out], rooms: params[:rooms], action_name: action_name } }).serialized_json, status: :ok
  end
