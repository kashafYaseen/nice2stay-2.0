    ...
    def accommodation_rules
      ...
      if active_rules.present?
        active_rules.each do |rule|
          errors.add(:check_in, "and checkout must be on #{lodging.check_in_day.try(:titleize)}") if lodging.check_in_day == rule.checkin_day && !rule.flexible_arrival && nights % 7 != 0
